// ══════════════════════════════════════════════
// SMART FIXER — CONNECTIVITY GRAPH BUILDER (Region B)
// ══════════════════════════════════════════════
...
export function buildConnectivityGraph(dataTable, config) {
  ...
  const snap = (coord) => {
    ...
    if (gridSnap === 0) return { x: coord.x, y: coord.y, z: coord.z };
    return {
  ...
  const components = dataTable
    .filter(row => {
      const t = row.type ? row.type.toUpperCase() : "";
      return t && !["ISOGEN-FILES", "UNITS-BORE", "UNITS-CO-ORDS",
        "UNITS-WEIGHT", "UNITS-BOLT-DIA", "UNITS-BOLT-LENGTH",
        "PIPELINE-REFERENCE", "MESSAGE-SQUARE"].includes(t);
    })
    .map(row => ({
      ...row,
